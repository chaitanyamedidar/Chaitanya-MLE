...
def list_subscriptions(db: Session = Depends(get_db)) -> list[SubscriptionRead]:
    return [svc.annotate(sub) for sub in svc.list_subscriptions(db)]


@router.get("/{subscription_id}", response_model=SubscriptionRead)
def get_subscription(subscription_id: int, db: Session = Depends(get_db)) -> SubscriptionRead:
    sub = svc.get_subscription(db, subscription_id)
    if sub is None:
